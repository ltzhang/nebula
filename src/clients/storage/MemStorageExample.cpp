...
class MemStorageExample {
 public:
  void runExample() {
    ...
    auto addVertexFuture = memClient.addVertices(param, std::move(vertices),
    ...
    auto addEdgeFuture = memClient.addEdges(param, std::move(edges),
                                          edgePropNames, false, false);
    ...
    auto getPropsFuture = memClient.getProps(param, inputDataSet, &vertexProps,
                                           nullptr, nullptr);
