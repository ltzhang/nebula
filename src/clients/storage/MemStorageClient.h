...
#ifndef CLIENTS_STORAGE_MEMSTORAGECLIENT_H
#define CLIENTS_STORAGE_MEMSTORAGECLIENT_H
...
template <typename T>
using MemStorageRpcRespFuture = folly::SemiFuture<StorageRpcResponse<T>>;

/**
 * A memory-based storage client that replaces Thrift calls with direct MemStore access
 */
class MemStorageClient {
 public:
  ...
  // Vertex operations
  MemStorageRpcRespFuture<cpp2::ExecResponse> addVertices(
  ...
  MemStorageRpcRespFuture<cpp2::ExecResponse> deleteVertices(
      const CommonRequestParam& param,
      std::vector<Value> ids);

  MemStorageRpcRespFuture<cpp2::ExecResponse> deleteTags(
      const CommonRequestParam& param,
  ...
  // Edge operations
  MemStorageRpcRespFuture<cpp2::ExecResponse> addEdges(
      const CommonRequestParam& param,
  ...
  MemStorageRpcRespFuture<cpp2::ExecResponse> deleteEdges(
      const CommonRequestParam& param,
  ...
  // Query operations
  MemStorageRpcRespFuture<cpp2::GetNeighborsResponse> getNeighbors(
      const CommonRequestParam& param,
  ...
  MemStorageRpcRespFuture<cpp2::GetDstBySrcResponse> getDstBySrc(
      const CommonRequestParam& param,
      const std::vector<Value>& vertices,
      const std::vector<EdgeType>& edgeTypes);

  MemStorageRpcRespFuture<cpp2::GetPropResponse> getProps(
      const CommonRequestParam& param,
      const DataSet& input,
      const std::vector<cpp2::VertexProp>* vertexProps,
      const std::vector<cpp2::EdgeProp>* edgeProps,
  ...
  // Scan operations
  MemStorageRpcRespFuture<cpp2::ScanResponse> scanVertex(
      const CommonRequestParam& param,
      const std::vector<cpp2::VertexProp>& vertexProp,
      int64_t limit,
      const Expression* filter);

  MemStorageRpcRespFuture<cpp2::ScanResponse> scanEdge(
      const CommonRequestParam& param,
 ...
 private:
  ...
  MemStore* memStore_{MemStore::instance()};
};

}  // namespace storage
}  // namespace nebula

#endif  // CLIENTS_STORAGE_MEMSTORAGECLIENT_H
